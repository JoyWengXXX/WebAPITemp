...
#region ORM資料庫連線
    #region EF Core
    builder.Services.AddDbContext<SignalRChatTemp.DBContexts.EFCore.ProjectDBContext_SignalR>(options =>
    {
    ...
    #region Dapper
    //註冊不同的DB連線
    var projectDBContext_1 = new SignalRChatTemp.DBContexts.Dapper.ProjectDBContext_SignalR(builder.Configuration.GetConnectionString("SignalRConnection"));
    var projectDBContext_2 = new SignalRChatTemp.DBContexts.Dapper.ProjectDBContext_Default(builder.Configuration.GetConnectionString("DefaultConnection"));
...
//註冊autofac這個容器
builder.Host.ConfigureContainer<ContainerBuilder>(builder => builder.RegisterModule(new AutofacModuleRegister("SignalRChatTemplete.Services", "Service", Assembly.GetExecutingAssembly())));
//註冊其他介面
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        var context = services.GetRequiredService<SignalRChatTemp.DBContexts.EFCore.ProjectDBContext_SignalR>();
